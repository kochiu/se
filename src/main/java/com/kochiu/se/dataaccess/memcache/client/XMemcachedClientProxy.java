 ...
 * 
 */
package com.kochiu.se.dataaccess.memcache.client;
...
public class XMemcachedClientProxy implements XMemcachedClientMBean, MemcachedClient {
	...
	private void connect(final InetSocketAddressWrapper inetSocketAddressWrapper) throws IOException {
		...
		for (int i = 0; i < this.connectionPoolSize; i++) {
			...
			try {
				...
				throwable = e;
				log.error("connect to " + SystemUtils.getRawAddress(inetSocketAddress) + ":" + inetSocketAddress.getPort() + " error", e);
			} catch (TimeoutException e) {
				throwable = e;
				log.error("connect to " + SystemUtils.getRawAddress(inetSocketAddress) + ":" + inetSocketAddress.getPort() + " timeout", e);
			} catch (Exception e) {
				throwable = e;
				log.error("connect to " + SystemUtils.getRawAddress(inetSocketAddress) + ":" + inetSocketAddress.getPort() + " error", e);
			}
			...
			if (!connected) {
				...
				log.error("Connect to " + SystemUtils.getRawAddress(inetSocketAddress) + ":" + inetSocketAddress.getPort() + " fail", throwable);
	...
	private void buildConnector(MemcachedSessionLocator locator, BufferAllocator bufferAllocator, Configuration configuration,
		...
		log.warn("XMemcachedClient is using " + this.commandFactory.getProtocol().name() + " protocol");
