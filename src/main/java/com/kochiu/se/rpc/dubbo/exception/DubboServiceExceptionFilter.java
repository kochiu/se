...
import com.pay1pay.framework.core.exception.BusinessCheckedException;
			...
			if (result.hasException() && GenericService.class != invoker.getInterface()) {
				try {
					...
					if (ex instanceof RpcException) {
						...
						return new RpcResult(new BusinessException(bex.getCode(), bex.getDescription(), bex.getResultMap(), bex.getCause()));
						...
						return new RpcResult(new SystemException(sex.getCode(), sex.getDescription(), sex.getResultMap(), sex.getCause()));
						...
						BusinessRuntimeException brex = (BusinessRuntimeException) ex;
						return new RpcResult(new BusinessRuntimeException(brex.getErrCode(), brex.getErrMsg(), brex.getAttach()));
					} else if (ex instanceof BusinessCheckedException) {
						BusinessCheckedException brex = (BusinessCheckedException) ex;
						return new RpcResult(new BusinessRuntimeException(brex.getErrCode(), brex.getErrMsg(), brex.getAttach()));
					}
