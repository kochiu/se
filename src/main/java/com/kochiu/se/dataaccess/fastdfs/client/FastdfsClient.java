package com.kochiu.se.dataaccess.fastdfs.client;
...
public class FastdfsClient {
	...
	public String upload(String filePath, String fileExt, Map<String, String> extraInfo) {
		...
		try {
			...
			throw new SystemException("Fastdfs upload file error, filePath=" + filePath, e);
		} finally {
			if (trackerServer != null) {
				try {
					trackerServer.close();
				} catch (IOException e) {
					throw new SystemException("Close trackerServer=" + trackerServer + " error", e);
				}
			}
		}
	}
	...
	public FileInfo download(String fileId) {
		...
		try {
			...
			return new FileInfo(fileId, content, size, author, extraInfo);
		} catch (Exception e) {
			throw new SystemException("Fastdfs download file error, fileId=" + fileId, e);
		} finally {
			if (trackerServer != null) {
				try {
					trackerServer.close();
		...
		if (fileInfo != null) {
			try {
				...
				if (!file.exists() || !file.isFile()) {
					throw new SystemException("Fastdfs download file save to " + localFilePath + " fail");
				}
			} catch (IOException e) {
				throw new SystemException("Fastdfs download file save to " + localFilePath + " fail", e);
			}
		}
	...
	public boolean deleteFile(String fileId) {
		...
		try {
			...
			throw new SystemException("Fastdfs delete file error, fileId=" + fileId, e);
		} finally {
			if (trackerServer != null) {
				try {
					trackerServer.close();
